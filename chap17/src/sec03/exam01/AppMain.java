...
public class AppMain extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        textField.setPrefWidth(200); // TextField 의 폭 설정

        Button button = new Button(); // Button 컨트롤 생성
        button.setText("OK"); // Button 글자 생성

        ObservableList list = hbox.getChildren(); // HBox 의 ObservableList 얻기
        list.add(textField); // TextField 컨트롤 배치
        list.add(button); // Button 컨트롤 배치

        Scene scene = new Scene(hbox); // 장면 생성

        stage.setTitle("AppMain"); // 윈도우 창 제목 설정
        stage.setScene(scene); // 윈도우 창에 장면 설정
