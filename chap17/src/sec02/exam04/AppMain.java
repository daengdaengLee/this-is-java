...
public class AppMain extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        root.setPrefWidth(350); // VBox 의 폭 설정
        root.setPrefHeight(150); // VBox 의 높이 설정
        ...
        button.setText("OK"); // text 속성 설정
        button.setOnAction(event -> Platform.exit());

        root.getChildren().add(label); // VBox 의 자식으로 Label 컨트롤 추가
        root.getChildren().add(button); // VBox 의 자식으로 Button 컨트롤 추가

        Scene scene = new Scene(root); // VBox 를 루트 컨테이너로 해서 Scene 생성
